...
public class CheckerTestUtil {
    ...
    public static List<ActualDiagnostic> getDiagnosticsIncludingSyntaxErrors(
        ...
        for (Pair<MultiTargetPlatform, BindingContext> binding : implementingModulesBindings) {
            MultiTargetPlatform platform = binding.getFirst();
            String platformString =
                    platform instanceof MultiTargetPlatform.Specific ? ((MultiTargetPlatform.Specific) platform).getPlatform() : null;

            result.addAll(getDiagnosticsIncludingSyntaxErrors(
                    binding.getSecond(), root, markDynamicCalls, dynamicCallDescriptors, platformString
            ));
        }
